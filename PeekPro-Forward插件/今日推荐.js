...
async function loadHistoryToday(params = {}) {
    ...
    const batchRequest = async (years) => {
        ...
        results.forEach(list => {
            if (list) allMovies = allMovies.concat(list);
        });
    };
...
async function fetchTmdbRecommendations(id, type) {
    try {
        const res = await Widget.tmdb.get(`/${type}/${id}/recommendations`, {
            params: {
                language: "zh-CN",
                page: 1
            }
...
async function fetchRandomTmdb(type) {
    ...
    try {
        const res = await Widget.tmdb.get(`/discover/${type}`, {
            params: queryParams
        });
